			...
			<div class="space-y-4">
				<accordion :title="note.title" v-for="(note, index) in notes" :key="index">
					<template #headerOptions>
						<svg v-if="index !== 0" @click="deleteNote(index)" xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-red-500 bg-red-200 p-1 rounded-full cursor-pointer" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
...
export default {
	...
	methods: {
		addNote(){
			this.notes.push({
				title: 'Notes ' + ((this.notes.length)+1),
				content: ''
			})
		},
		deleteNote(index){
